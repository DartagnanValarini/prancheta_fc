...
/* ====================================================================
   OBJETIVOS (A3) — modelo híbrido
   • PRINCIPAL: meta da diretoria, derivada do formato da divisão. Avaliada
     no FIM da temporada. Cumprir dá bônus grande + confiança; falhar tira
     confiança. Conversa com o sistema de confiança existente.
   • SECUNDÁRIOS: 3 desafios rotativos sorteados por temporada, com progresso
     a cada rodada e pagamento ao concluir. São o gancho dos ads recompensados
     (Bloco B): concluiu → oferta de dobrar a recompensa vendo um anúncio.
   Estado vive em App.objetivos (persistido no save). Nada aqui é re-derivável
   de outra fonte, então é memória de jogo legítima.
   ==================================================================== */
const Objetivos={
  // bônus base (em reais) escalado por divisão — divisões de cima pagam mais
  escalaDiv(div){ return ({A:3, B:2.2, C:1.6, D:1.1})[div]||1.5; },

  // ---- PRINCIPAL: meta da diretoria conforme a divisão ----
  principalDe(div){
    const esc=this.escalaDiv(div);
    if(div==='A') return {id:'ficar_a', tipo:'principal',
      titulo:'Permanecer na Série A', desc:'Termine fora da zona de rebaixamento (Z-4).',
      premio:Math.round(6e6*esc), confOk:8, confFail:-14};
    if(div==='D') return {id:'subir_d', tipo:'principal',
      titulo:'Subir para a Série C', desc:'Avance no mata-mata e conquiste o acesso.',
      premio:Math.round(6e6*esc), confOk:12, confFail:-10};
    // B e C: subir é a meta; não cair é o piso
    const acima=({B:'Série A',C:'Série B'})[div];
    return {id:'subir_'+div.toLowerCase(), tipo:'principal',
      titulo:'Conquistar acesso à '+acima, desc:'Termine entre os primeiros e suba de divisão.',
      premio:Math.round(6e6*esc), confOk:12, confFail:-12};
  },

  // ---- SECUNDÁRIOS: catálogo. metaDe() dá o alvo; cada um sabe se progride. ----
  // progresso é acumulado em App.objetivos.sec[i].prog pelos eventos da rodada.
  CATALOGO:[
    {id:'vit_casa',  titulo:'Fortaleza em casa',  descF:m=>`Vença ${m} jogos como mandante.`,      meta:[3,4,5],   ev:'vitCasa'},
    {id:'gols_temp', titulo:'Ataque avassalador',  descF:m=>`Marque ${m} gols na temporada.`,        meta:[18,25,32],ev:'gols'},
    {id:'invicto',   titulo:'Invencível',          descF:m=>`Fique ${m} rodadas seguidas sem perder.`,meta:[4,5,6],  ev:'invicto', streak:true},
    {id:'clean',     titulo:'Muralha',             descF:m=>`Não sofra gols em ${m} jogos.`,          meta:[3,4,5],   ev:'clean'},
    {id:'vitorias',  titulo:'Máquina de vencer',   descF:m=>`Vença ${m} jogos na temporada.`,         meta:[8,11,14], ev:'vitoria'},
  ],

  // sorteia 3 secundários distintos, com dificuldade/prêmio escalados pela divisão
  sortearSecundarios(div){
    const esc=this.escalaDiv(div);
    const pool=[...this.CATALOGO];
    // embaralha
    for(let i=pool.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [pool[i],pool[j]]=[pool[j],pool[i]]; }
    return pool.slice(0,3).map(c=>{
      const nivel=Math.floor(Math.random()*c.meta.length);      // 0=fácil,1=médio,2=difícil
      const meta=c.meta[nivel];
      const premio=Math.round((0.8e6 + nivel*0.6e6)*esc);         // mais difícil paga mais
      return {id:c.id, ev:c.ev, streak:!!c.streak, titulo:c.titulo,
              desc:c.descF(meta), meta, prog:0, feito:false, premioBase:premio, adUsado:false};
    });
  },

  // (re)inicia os objetivos da temporada para a divisão atual
  novaTemporada(div){
...
const App={
  ...
  // escapa texto pra render seguro em innerHTML. Nomes de jogador hoje são gerados
  // pelo jogo, mas nome de clube/usuário (Bloco C) virá do usuário — usar sempre.
  ...
  /* ---------- ESTATÍSTICAS DA TEMPORADA (A2) ----------
     Agrega jogadores da MINHA divisão a partir de dados que já existem
     (golsTemp do motor, notas do A1). Tudo re-derivável — nada persistido novo. */

  // todos os jogadores da minha divisão, com o time anexado, para rankings
  _jogadoresDaMinhaDivisao(){
    const div=this.divisao;
    const out=[];
    this.indicesDaDivisao(div).forEach(ti=>{
      const t=this.teams[ti]; if(!t) return;
      t.players.forEach(p=>out.push({p, ti, timeNome:t.nome, timeAbrev:t.abrev}));
    });
    return out;
  },
  // top artilheiros da temporada (gols > 0), maior primeiro
  topArtilheiros(n=10){
    return this._jogadoresDaMinhaDivisao()
      .filter(o=>(o.p.golsTemp||0)>0)
  ...
  topNotas(n=10, minJogos=3){
    return this._jogadoresDaMinhaDivisao()
      .filter(o=>(o.p._qtdNotas||0)>=minJogos)
      .map(o=>({...o, media:o.p._somaNotas/o.p._qtdNotas}))
      .sort((a,b)=>b.media-a.media)
      .slice(0,n);
  },
  // craque da última rodada do MEU time (setado no encerrarRodada)
  craqueDaRodada(){ return this._craqueRodada||null; },

  // A2: sparkline SVG de uma série numérica (sem libs). vals = array de números.
  sparkline(vals, cor){
    const n=vals.length;
    if(n<2) return '<div style="color:var(--muted);font-size:.85em;padding:8px 0">Um gráfico aparece a partir da 2ª temporada.</div>';
    ...
    const c=cor||'var(--lemon)';
    return `<svg viewBox="0 0 ${W} ${H}" width="100%" height="${H}" preserveAspectRatio="none" style="display:block">
      <polyline points="${pts}" fill="none" stroke="${c}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>
      <circle cx="${x(n-1).toFixed(1)}" cy="${y(vals[n-1]).toFixed(1)}" r="3" fill="${c}"/>
    </svg>`;
  },

  /* ---------- OBJETIVOS (A3) ---------- */
  garantirObjetivos(){
    if(!this.objetivos || this.objetivos.div!==this.divisao){
      this.objetivos=Objetivos.novaTemporada(this.divisao);
    }
    return this.objetivos;
  },
  registrarEventosObjetivos(ev){
    const o=this.garantirObjetivos();
    o.sec.forEach(s=>{
      if(s.feito) return;
      let inc=0;
      switch(s.ev){
        case 'vitCasa': if(ev.resultado==='v' && ev.souCasa) inc=1; break;
        case 'vitoria': if(ev.resultado==='v') inc=1; break;
        case 'gols':    inc=ev.meusGols||0; break;
        case 'clean':   if(ev.golsAdv===0) inc=1; break;
        case 'invicto': if(ev.resultado==='d'){ s.prog=0; } else { s.prog=(s.prog||0)+1; }
          if(s.prog>=s.meta) this.concluirObjetivoSec(s);
          return;
      }
      if(inc>0){ s.prog=Math.min(s.meta,(s.prog||0)+inc); if(s.prog>=s.meta) this.concluirObjetivoSec(s); }
    });
  },
  concluirObjetivoSec(s){
    if(s.feito) return;
    s.feito=true; s.prog=s.meta;
    const t=this.teams[this.myTeam]; if(t){ t.saldo=(t.saldo||0)+s.premioBase; this.addExtrato('Objetivo: '+s.titulo, +s.premioBase); }
    if(!Array.isArray(this._objetivosConcluidos)) this._objetivosConcluidos=[];
  ...
  avaliarPrincipal(destino){
    ...
    const subi = destino && destino.sobem && destino.sobem.includes(eu);
    const cai  = destino && destino.caem  && destino.caem.includes(eu);
    let ok=false;
    if(P.id==='ficar_a')       ok = !cai;
    else if(P.id==='subir_d')  ok = subi;
    else                       ok = subi;
    o.principalStatus = ok?'cumprido':'falhou';
    if(this.confianca!=null) this.confianca=Math.max(0,Math.min(100,this.confianca+(ok?P.confOk:P.confFail)));
    if(ok){ const t=this.teams[eu]; if(t){ t.saldo=(t.saldo||0)+P.premio; this.addExtrato('Meta da diretoria: '+P.titulo, +P.premio); } }
    return {ok, premio:ok?P.premio:0, titulo:P.titulo};
  },

  // A2: retrato do meu clube nesta temporada (pra série histórica)
  retratoClube(){
    const t=this.teams[this.myTeam]; if(!t) return null;
    const jog=t.players||[];
    const ovs=jog.map(p=>Rating.overallGlobal(p)).filter(v=>v>0);
    const ovMedio = ovs.length ? Math.round(ovs.reduce((a,b)=>a+b,0)/ovs.length) : 0;
    const valorElenco = jog.reduce((a,p)=>a+(this.valorDe?this.valorDe(p):0),0);
    return { temporada:this.temporada||1, divisao:this.divisao,
             saldo:Math.round(t.saldo||0), valorElenco:Math.round(valorElenco), ovMedio };
  },
  // registra o retrato no histórico (idempotente por temporada)
  registrarHistoricoClube(){
    const r=this.retratoClube(); if(!r) return;
    if(!Array.isArray(this.histClube)) this.histClube=[];
    if(this.histClube.some(h=>h.temporada===r.temporada)) return; // já registrado
    this.histClube.push(r);
    if(this.histClube.length>50) this.histClube=this.histClube.slice(-50); // teto
  },
  ...
  renderCompeticoes(){
    ...
    el.innerHTML=`
      <div class="cmp-tabs">
        ...
        <button class="cmp-tab ${aba==='stats'?'on':''}" data-cmp="stats">📊 Estatísticas</button>
      </div>
    ...
    if(aba==='stats'){
      const arts=this.topArtilheiros(10);
      const notas=this.topNotas(10,3);
      const craque=this.craqueDaRodada();
      // A3: painel de objetivos da temporada
      const obj=this.garantirObjetivos();
      const P=obj.principal;
      const barraObj=(prog,meta)=>{ const pct=Math.min(100,Math.round((prog/meta)*100));
        return `<div class="obj-bar"><div class="obj-bar-fill" style="width:${pct}%"></div></div>`; };
      const secHTML=obj.sec.map(s=>`
        <div class="obj-card ${s.feito?'obj-done':''}">
          <div class="obj-card-h">${s.feito?'✅':'🎯'} ${this.esc(s.titulo)}</div>
          <div class="obj-desc">${this.esc(s.desc)}</div>
          ${barraObj(s.prog,s.meta)}
          <div class="obj-foot">
            <span class="obj-prog">${Math.min(s.prog,s.meta)}/${s.meta}</span>
            ${s.feito
              ? (s.adUsado
      ...
      const principalHTML=`
        <div class="obj-card obj-principal">
          <div class="obj-card-h">🏛️ Meta da diretoria — ${this.esc(P.titulo)}</div>
          <div class="obj-desc">${this.esc(P.desc)}</div>
          <div class="obj-foot"><span class="obj-premio">Prêmio ${this.fmtM(P.premio/1e6)} + confiança</span>
            <span class="obj-prog">${obj.principalStatus==='aberto'?'Em andamento':(obj.principalStatus==='cumprido'?'✅ Cumprida':'❌ Falhou')}</span></div>
        </div>`;
      // A2: série histórica + temporada corrente (ao vivo)
      const serie=[...(this.histClube||[])];
      const atual=this.retratoClube();
      if(atual && !serie.some(h=>h.temporada===atual.temporada)) serie.push(atual);
      const linhaArt=(o,pos)=>`<div class="cmp-row ${o.ti===this.myTeam?'eu':''}">
        <span class="cmp-pos">${pos+1}º</span>
      ...
      const linhaNota=(o,pos)=>`<div class="cmp-row ${o.ti===this.myTeam?'eu':''}">
        <span class="cmp-pos">${pos+1}º</span>
        <span class="cmp-nome">${this.esc(o.p.nome)} <span style="color:var(--muted);font-size:.85em">${o.timeAbrev||''}</span>${flag(o.ti)}</span>
        <span class="cmp-pts" style="color:${this.corNota(o.media)}">${o.media.toFixed(1)}</span>
      </div>`;
      body.innerHTML=`
        ...
        <div class="cmp-grid">
          <div class="cmp-grupo meu" style="grid-column:1/-1">
            <div class="cmp-grupo-h">📉 Evolução do clube <span style="color:var(--muted);font-size:.8em">(por temporada)</span></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;padding:10px 12px">
              <div><div style="color:var(--muted);font-size:.8em;margin-bottom:2px">Saldo — atual ${this.fmtM((atual?atual.saldo:0)/1e6)}</div>${this.sparkline(serie.map(h=>h.saldo),'var(--lemon)')}</div>
              <div><div style="color:var(--muted);font-size:.8em;margin-bottom:2px">Overall médio — atual ${atual?atual.ovMedio:'–'}</div>${this.sparkline(serie.map(h=>h.ovMedio),'#e8c547')}</div>
            </div>
          </div>
          ...
          <div class="cmp-grupo">
            ...
            ${notas.length?notas.map(linhaNota).join(''):'<div class="cmp-nota">Notas aparecem após alguns jogos disputados.</div>'}
          </div>
        </div>`;
      // A3: gancho de anúncio (Bloco B) — dobra o prêmio do objetivo concluído
      body.querySelectorAll('[data-objad]').forEach(b=>b.onclick=()=>{
        this.assistirAnuncioBonus(b.dataset.objad);
      });
      return;
    }
  ...
  novaTemporadaCompleta(){
    this.registrarHistoricoClube();   // A2: fecha o retrato da temporada que terminou
    // A3: avalia a meta da diretoria ANTES de trocar de divisão (usa o destino da minha liga)
    const minhaLiga=this.ligas&&this.ligas[this.divisao];
    if(minhaLiga){ this._resultadoPrincipal=this.avaliarPrincipal(this.destinosDaLiga(minhaLiga)); }
